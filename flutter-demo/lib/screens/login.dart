...
class Login extends StatelessWidget {
  ...
  final redirectUri = "https://edit-with-rahul-community.avalonmeta.com/oauth2/callback";
  final communityId = "61";
  // final cookieManager = WebviewCookieManager();

  Future<ScenesResponse> getScenesToken(String code) async {
    const String v4 = 'https://scenes-ruby-staging.avalonmeta.com/api/v4/users/oauth/custom_login_app?community_id=61';
  ...
  Future<LoginResponse> getUrlAfterLogin() async {
    final String url = 'https://custom-sso.herokuapp.com/auth/login?client_id=$clientId&redirect_uri=$redirectUri&response_type=code';
  ...
  void handleLogin(context) async {
    ...
    LoginResponse res =  await getUrlAfterLogin();
    print(res.code);
    ScenesResponse scenesResponse = await getScenesToken(res.code);
    // List<Cookie> cookies;
    final String cookieString = '{"token":"${scenesResponse.token}","refreshToken":"${scenesResponse.refreshToken}","userId":"${scenesResponse.userId}"}';

    final _cookieManager = CookieManager();
    WebViewCookie cookie = WebViewCookie(name: 'custom-login-auth', value: cookieString, domain: Constants().communityDomain);
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              CupertinoButton(onPressed: () => handleLogin(context), child: const Text("Login"))
            ],
          ),
        ),
      ),
